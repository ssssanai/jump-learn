...
<div class="wrap">
    ...
    <div class="main">
        <div class="writeTit">
            <h2>대입 정보 게시판</h2>
            <p>대학 관련 정보를 손쉽게 얻을 수 있는 게시판입니다.</p>
            <div class="wtBtn">
                <c:if test="${isAdmin}">
                    <a href="/info/writePage">글쓰기</a>
                </c:if>

            </div>
        </div>
        <form class="searchInput" name="frmSearch" id="frmSearch" method="GET" action="/info/searchListPage">
            <input type="hidden" name="page_size" value="${pageDTO.page_size}" />
            <div class="siReset">
                <input type="button" onclick="location.href='<c:url value="/info/searchListPage"/>'"  value="검색조건 초기화">
            </div>
        ...
        <div class="writeList">
            ...
            <c:forEach var="post" items="${dto.dtoList}">
                <div class="wlBody">
                    ...
                    <p class="listTit"><a href="/info/viewPage?id=${post.id}&${pageDTO.getLinkParams()}">${post.title}</a></p>
                    ...
                    <c:if test="${isAdmin}">
                        <div class="listDelBtnBox">
                            <input class="listDelBtn" type="button" id="deleteBtn${post.id}" name="deleteBtn${post.id}" onClick="if(confirm('${post.title} 글을 삭제하시겠습니까?')) {location.href='/info/delete/${post.id}'}" value="삭제">
                        </div>
                    </c:if>
                </div>
            </c:forEach>
