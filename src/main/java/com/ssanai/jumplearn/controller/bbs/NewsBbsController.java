...
@Controller
@RequestMapping("/news")
public class NewsBbsController {
	...
	public String writePagePOST(
		...
		try {
			int result = bbsService.insert(dto, "tbl_news");
			if (result < 0) {
				log.info("글 등록 실패");
				redirectAttributes.addFlashAttribute("msg", "글 등록에 실패했습니다.");
				return "redirect:/news/writePage?" + pageDTO.getLinkParams();
			}
		...
		return "redirect:/news/searchListPage?" + pageDTO.getLinkParams();
	}
	...
	public String viewPage(
		...
		bbsService.viewCount(id, "tbl_news");
		...
		BbsDefaultDTO dto = bbsService.selectOne(id, "tbl_news");

		model.addAttribute("dto", dto);
		model.addAttribute("pageDTO", pageDTO);
		return "news/viewPage";
	}
	...
	public String editPage(
		...
		log.info("adto", adto.toString());
		model.addAttribute("adto", adto);
		model.addAttribute("dto", bbsService.selectOne(id, "tbl_news")); // 게시글 정보
		model.addAttribute("pageDTO", pageDTO);
	...
	public String editPagePOST(
		...
		bbsService.update(dto, "tbl_news");
		return "redirect:/news/searchListPage?" + pageDTO.getLinkParams();
	}
	...
	public String searchListPage(
		...
		model.addAttribute("pageDTO", pageDTO);
		PageResponseDTO<BbsDefaultDTO> dto = bbsService.searchList(pageDTO, "tbl_news");
		int totalCount = bbsService.getTotalCount(pageDTO, "tbl_news");
