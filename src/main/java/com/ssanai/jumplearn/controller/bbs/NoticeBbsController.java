...
public class NoticeBbsController {
	...
	public String writePagePOST(
		...
		try {
			int result = bbsService.insert(dto, "tbl_notice");
			if (result < 0) {
				log.info("글 등록 실패");
				redirectAttributes.addFlashAttribute("msg", "글 등록에 실패했습니다.");
				return "redirect:/notice/writePage?" + pageDTO.getLinkParams();
			}
		...
		return "redirect:/notice/searchListPage?" + pageDTO.getLinkParams();
	}
...
//		model.addAttribute("adminInfo", adminInfo);
	...
	public String editPagePOST(
		...
		bbsService.update(dto, "tbl_notice");
		return "redirect:/notice/searchListPage?" + pageDTO.getLinkParams();
	}
	...
	public String delete(
		...
		bbsService.delete(id, "tbl_notice");
		return "redirect:/notice/searchListPage?" + pageDTO.getLinkParams();
	}
